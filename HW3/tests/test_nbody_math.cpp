// Standalone numeric validation of the AVX2 nbody kernel vs the serial
// reference. Replicates the exact SIMD math from nbody_impl.h but uses a
// plain loop instead of tbb::parallel_for (so no TBB dependency is needed).
// Verifies results stay within the grader's 0.1 epsilon.
#include "../nbody.h"     // serial reference (particles[], move_particles_serial)
#include <immintrin.h>
#include <cstdio>
#include <cmath>
...
static void move_par() {
    ...
    for (int blk = 0; blk < nParticles / 4; blk++) {
        ...
        for (int j = 0; j < nParticles; j += 8) {
            ...
            #define DO_ONE(XIS, YIS, ZIS, FX, FY, FZ) \
                { \
            ...
            DO_ONE(xi0s, yi0s, zi0s, Fx0, Fy0, Fz0)
            DO_ONE(xi1s, yi1s, zi1s, Fx1, Fy1, Fz1)
            DO_ONE(xi2s, yi2s, zi2s, Fx2, Fy2, Fz2)
            DO_ONE(xi3s, yi3s, zi3s, Fx3, Fy3, Fz3)
            #undef DO_ONE
        }
...
int main() {
    ...
    init_par();
    const int steps = 10;
    for (int s = 0; s < steps; s++) { move_particles_serial(); move_par(); }
    ...
    printf("steps=%d  max abs pos error = %g (at i=%d)\n", steps, maxerr, worst);
    printf(maxerr < 0.1f ? "NBODY MATH OK (within 0.1 epsilon)\n" : "NBODY MATH FAIL\n");
    return maxerr < 0.1f ? 0 : 1;
}
